...
class Simulation {
public:
	Simulation(std::vector<Sand> particles, float dt);
	~Simulation() {};
	...
	void add_particle(Sand water) {
			particles_.push_back(water);
	}

	int get_grid_index_2d(int x, int y);

	float get_wip(const glm::vec2& deltax_ip)
	{
		return kernel_(deltax_ip.x) * kernel_(deltax_ip.y);
	}

	//equation (2) (3)
	glm::vec2 get_gradient_wip(const glm::vec2& deltax_ip)
	{
		return glm::vec2(kernel_gradient_(deltax_ip.x) * kernel_(deltax_ip.y), kernel_(deltax_ip.x) * kernel_gradient_(deltax_ip.y));
	}

private:
	std::vector<Sand> particles_;
