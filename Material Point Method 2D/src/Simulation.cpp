...
//=======================Utilities=======================================//
int Simulation::get_grid_index_2d(int x, int y)
{
return x + gridnode_->grid_num_x * y;
}
...
Simulation::Simulation(std::vector<Sand> particles, float dt) {
	borders_ = std::make_shared<Border>();
	gridnode_ = std::make_shared<GridNode>();
	particles_ = particles;
...
#pragma omp parallel for 
	for (size_t i = 0; i < particles_.size(); i++) {
		...
		int index0 = get_grid_index_2d(x0, y0);

		//water
		//float force = particles_[i].compute_force_per_particle();
		//sand
		glm::mat2 force = particles_[i].compute_force_per_particle();
		...
		for (int y = y0 -1 ; y < y0 + 3; y++) {
			...
			{
				int index = get_grid_index_2d(x, y); //grid index

				//add weight to particle for numerical purpose
				glm::vec2 xpmxi = particles_[i].xp_ - gridnode_->grids_[index].xi_;
				float wip = get_wip(xpmxi);
				glm::vec2 dwip = get_gradient_wip(xpmxi);
				...
				glm::mat2 Dp_inv = 3.0f * glm::mat2(1.0f);
				glm::vec2 v_ip = wip * particles_[i].mp_ * (particles_[i].vp_ + particles_[i].Bp_ * Dp_inv * (-xpmxi)); //NOTE: not divided by mass here!!!
				glm::vec2 f_ip = - force * dwip; //NO gravity !!!!
...
{
	const glm::vec2 G = glm::vec2(0.0f, -10.0f);
...
#pragma omp parallel for 
	for (int i = 0; i < particles_.size(); i++){
		int x0 = int(particles_[i].xp_.x);
		int y0 = int(particles_[i].xp_.y);
		int index0 = get_grid_index_2d(x0, y0); //grid index based on the particle location
		...
		for (int y = y0 - 1; y < y0 + 3; y++) {
			for (int x = x0 - 1; x < x0 + 3; x++)
			{
				...
				int wip_index = (y - y0 + 1) * 4 + (x - x0 + 1);
				float wip = particles_[i].wip_list_[wip_index];
				glm::vec2 xpmxi = particles_[i].ximxp_list_[wip_index];
...
#pragma omp parallel for 
	for (int i = 0; i < particles_.size(); i++){
		...
		glm::vec2 Xp_buff = particles_[i].xp_;
		glm::vec2 xpnp1(0.0f);
		glm::mat2 gradient_vp(0.0f);
		...
		for (int y = y0 - 1; y < y0 + 3; y++) {
			for (int x = x0 - 1; x < x0 + 3; x++)
			{
				...
				int wip_index = (y - y0 + 1) * 4 + (x - x0 + 1);
				float wip = particles_[i].wip_list_[wip_index];
				glm::vec2 dwip = particles_[i].dwip_list_[wip_index];
		...
		//float trace_gradient_vp = gradient_vp[0][0] + gradient_vp[1][1];
		//particles_[i].Jp_ = (1.0f + dt_ * trace_gradient_vp) * particles_[i].Jp_;
		//Sand
		particles_[i].UpdateDeformation(gradient_vp, dt_);
	}
...
#pragma omp parallel for schedule (dynamic)
	while (it != active_grids_.end())
	{
		...
		gridnode_->grids_[i].vi_ = glm::vec2(0.0f);
		gridnode_->grids_[i].vi_star_ = glm::vec2(0.0f);
		gridnode_->grids_[i].vi_col_ = glm::vec2(0.0f);
		gridnode_->grids_[i].vi_fri_ = glm::vec2(0.0f);
		gridnode_->grids_[i].fi_ = glm::vec2(0.0f);
		it++;
	}
...
void Simulation::render_particles(Shader& shader, std::shared_ptr<Mesh> sphere, glm::mat4 projection, glm::mat4 view) {
	...
	for (int i = 0; i < p_num; i++) {
		...
		glm::vec3 translation(0.1f * glm::vec3(particles_[i].xp_, 0.0f));
...
void Simulation::render_borders(Shader& shader, glm::mat4 projection, glm::mat4 view) {
	float vertices[] = {
		 0.2f, 0.2f, 0.0f, // left  
		 9.8f, 0.2f, 0.0f, // right 
		 9.8f, 9.8f, 0.0f, // left  
		 0.2f, 9.8f, 0.0f, // right 
	};
