...
struct Grid {
	...
	void apply_collision(std::vector<Plane> planes, float dt) {
		...
		for (int b = 0; b < planes.size(); b++) {
			...
			glm::vec3 x_np1 = xi_ + dt * vi_star_;
	...
	void apply_friction(std::vector<Plane> planes) {
		...
		for (int b = 0; b < planes.size(); b++) {
			...
			glm::vec3 vi_n = glm::dot(planes[b].normal_, vi_col_) * planes[b].normal_;
			glm::vec3 vi_t = vi_col_ - vi_n;
			glm::vec3 t = glm::normalize(vi_t);
...
public:
	GridNode() {
		for (int k = 0; k < grid_num_z; k++) {
			for (int j = 0; j < grid_num_y; j++) {
				for (int i = 0; i < grid_num_x; i++) {
					Grid grid;
					grid.mi_ = 0.0f;
					grid.vi_ = glm::vec3(0.0f);
					grid.vi_star_ = glm::vec3(0.0f);
					grid.vi_col_ = glm::vec3(0.0f);
					grid.vi_fri_ = glm::vec3(0.0f);
					grid.fi_ = glm::vec3(0.0f);
					grid.xi_ = glm::vec3((float)i, (float)j, (float)k);
	...
	const int grid_num_y = 10;
	const int grid_num_z = 101;
private:
};
#endif
